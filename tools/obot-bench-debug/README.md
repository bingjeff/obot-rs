...
For a repeatable bring-up check, `check-driver-usb` safely sends a disable first, sends configure-enable, polls the USB text API for the main-loop DRV result, prints the DRV plus safety gates in one row, then sends disable again by default so a powered-ready probe does not leave the DRV enabled. Use `--expect unpowered-fail-closed` for the no-VM diagnostic state, or `--expect powered-ready` for the future VM-supplied gate that must pass before considering bridge output enablement. Use `--leave-driver-enabled` only for deliberate follow-on diagnostics that require the DRV to remain enabled after the row is captured.
